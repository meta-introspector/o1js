...
import { test as local_test, Random, withHardCoded } from '../testing/property.js';
//import {expect, jest, test as jest_test} from '@jest/globals';
import { each } from 'test-each'

each (10000, Math.random,
    ({title},{}) => {
	describe('base58', ()=> {
	    test('some wrapper', async () => {
		...
		local_test(bytes, version, (bytes, version, assert) => {
